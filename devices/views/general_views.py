...
def display_approved_imports(request):
    ...
    # ORDER BY PK DESCENDING (newest first)
...
def display_disposed_imports(request):
    ...
    this_month_count = Import.objects.filter(
        is_disposed=True,
        date__year=timezone.now().year,
        date__month=timezone.now().month
    ).count()

    data = base_disposed_data
...
def export_to_excel(request):
    ...
    # ---- headers ----------------------------------------------------------------
    headers = [
        'Centre', 'Department', 'Category', 'Hardware', 'System Model',
...
from ..utils import unknown_hardware_q
...
HARDWARE_CATEGORIES = {
    ...
    'routers/switch/server': Q(hardware__icontains='router') | Q(hardware__icontains='switch') | Q(hardware__icontains='server'),
    'n-computing': Q(hardware__icontains='N-Computing'),
...
def dashboard_view(request):
    ...
    # === NEW: Group by CATEGORY instead of parsing hardware string ===
    category_counts = (
        device_query
...
def filtered_list_view(request, list_type):
    ...
    if list_type == 'devices':
        ...
        # NEW: Category filter (replaces old hardware string matching)
